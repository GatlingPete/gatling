...
The definition of the injection profile of users is done with the `injectOpen` and `injectClosed` methods (just `inject` in Scala). These methods trigger a series of injection steps that are triggered sequentially.
...
* call centers, where all operators are busy
...
In such a case, when the system under test starts to have some trouble, response times will increase, journey time will become longer, so as the number of concurrent users increases, injectors will slow down to match the imaginary cap you've set.
{{< /alert >}}

You can read more about open and closed models [here](https://www.usenix.org/legacy/event/nsdi06/tech/full_papers/schroeder/schroeder.pdf) and in [our blog](https://gatling.io/2018/10/04/gatling-3-closed-workload-model-support/).
...
7. `rampUsersPerSec(rate1).to(rate2).during(duration).randomized`: Injects users from a starting rate to a target rate, defined in users per second, during a given duration. Users will be injected at randomized intervals.
...
1. `constantConcurrentUsers(nbUsers).during(duration)`: Injects users so that the number of concurrent users in the system is constant.
2. `rampConcurrentUsers(fromNbUsers).to(toNbUsers).during(duration)`: Injects users so that number of concurrent users in the system ramps linearly from one number to another

{{< alert warning >}}
Ramping down the number of concurrent users won't force the existing users to interrupt.
The only way for virtual users to terminate is for them to complete their scenario.
{{< /alert >}}

## Meta DSL

It is possible to use elements of the Meta DSL to write your tests in an easier way.
